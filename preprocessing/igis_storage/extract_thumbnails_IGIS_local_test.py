    ...
    with tqdm(total=total_annotations, desc="Extracting Thumbnails", unit=" annotations") as pbar:
        with open(csv_file, "r") as f:
            ...
            for i, row in enumerate(reader):
                ...
                # Load the full image using GPU-accelerated decoding
                full_image_path = os.path.join(full_images_dir, upload_id, file_name)
                gpu_image.upload(cv2.imread(full_image_path))

                # Perform GPU-accelerated cropping
                gpu_thumbnail = gpu_image[y_min:y_max+1, x_min:x_max+1]
                ...
                thumbnail_filename = f"{plant_id}_{thumb_id}.png"
                thumbnail_path = os.path.join(output_plant_dir, thumbnail_filename)
                cv2.imwrite(thumbnail_path, thumbnail)

full_images_dir = "full_images_test"
csv_file = "csv_data_IGIS/csv_sample_data.csv"
